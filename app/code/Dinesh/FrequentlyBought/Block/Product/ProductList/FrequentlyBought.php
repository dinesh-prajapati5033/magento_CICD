...
namespace Dinesh\FrequentlyBought\Block\Product\ProductList;
...
class FrequentlyBought extends Related
{
    ...
    public function getCustomOption($productId = null)
    {
        ...
        $option  = $this->getLayout()->getBlock('dinesh.frequently.bought.product.info.options')
            ->setProduct($product)
            ->toHtml();
        ...
        switch ($productType) {
            case 'configurable':
                $html = $this->getLayout()->createBlock(
                    \Dinesh\FrequentlyBought\Block\Product\View\Type\Configurable::class
                );
                break;
            case 'grouped':
                $html = $this->getLayout()->createBlock(\Magento\GroupedProduct\Block\Product\View\Type\Grouped::class)
                    ->setTemplate('Dinesh_FrequentlyBought::product/view/type/grouped.phtml');
                break;
            case 'bundle':
                $html = $this->getLayout()->getBlock('dinesh.fbt.product.info.bundle.options');
                break;
            case 'downloadable':
                $html = $this->getLayout()->createBlock(\Magento\Downloadable\Block\Catalog\Product\Links::class)
                    ->setTemplate('Dinesh_FrequentlyBought::product/view/type/downloadable/links.phtml');
                break;
        }
        if ($html) {
