...
namespace Dinesh\HyvaFrequentlyBought\Block\Product\ProductList;
...
use Dinesh\FrequentlyBought\Helper\Data as FbtData;
use Magento\Framework\View\DesignInterface;

class FrequentlyBought extends \Dinesh\FrequentlyBought\Block\Product\ProductList\FrequentlyBought
{
    /**
    ...
    public function getCustomOption($productId = null)
    {
        if (!$this->isHyva()) {
            ...
            $option  = $this->getLayout()->getBlock('dinesh.product.info.options')
                ->setProduct($product)
                ->toHtml();
            if ($option != '') {
                return $option;
            ...
            switch ($productType) {
                case 'configurable':
                    $html = $this->getLayout()->createBlock(
                        \Dinesh\HyvaFrequentlyBought\Block\Product\View\Type\Configurable::class
                    );
                    break;
                case 'grouped':
                    $html = $this->getLayout()
                        ->createBlock(\Magento\GroupedProduct\Block\Product\View\Type\Grouped::class)
                        ->setTemplate('Dinesh_HyvaFrequentlyBought::product/view/type/grouped.phtml');
                    break;
                case 'bundle':
                    $html = $this->getLayout()->getBlock('dinesh.product.info.bundle.options');
                    break;
                case 'downloadable':
                    $html = $this->getLayout()
