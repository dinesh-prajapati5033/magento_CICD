define([
    ...
    $.widget('mageants.frequentlyBought', {
        options: {
            addWishlistUrl: '',
        ...
        _bind: function() {
            this.element.find('.mageants-fbt-rows .related-checkbox').trigger('click');
        },

        _EventListener: function() {
            var self = this;
            this.element.on('change', '.mageants-fbt-rows .related-checkbox', function() {
                self._reloadTotalPrice();
            });
            this.element.on('change', '.mageants-fbt-rows .mageants-fbt-grouped .mageants-fbt-grouped-qty', function() {
                self._reloadGroupedPrice($(this));
            });
            this.element.find('.mageants-fbt-rows .product-custom-option').on('change', this._onOptionChanged.bind(this));
            this.element.on('click', '.mageants-fbt-add-to-wishlist button', function() {
                self._addToWishList($(this));
            });
            this.element.on('click', '.mageants-fbt-detail .detailed-node', function() {
                self._showHideDetail($(this));
            });
            this.element.on('click', '.mageants-fbt-add-to-cart button', function() {
                self._addValidation();
            });
        },

        _addValidation: function() {
            $('.mageants-fbt-rows ol li').each(function() {
                var selectTypesFlag = false,
                    $widget = $(this);
                if (!$widget.find('.related-checkbox').is(':checked')) {
                    $widget.find('.mageants-fbt-detail a:not(".not-active")').click();
                    return;
                }

                if ($(this).find('.mageants-fbt-grouped').length > 0) {
                    var active = false;
                    $(this).find('.mageants-fbt-grouped-qty').each(function() {
                    ...
                    if (!active) {
                        $(this).find('.mageants-fbt-detail a.not-active').click();
                    }
                }
                ...
                if (!selectTypesFlag && $(this).find('.mageants-fbt-grouped').length == 0) {
                    $widget.find('.mageants-fbt-detail a.not-active').click();
                }
            });
        },
        ...
        _onOptionChanged: function(event) {
            var optionPrice = 0,
                ...
                productId = checkboxElement.attr('data-mageants-fbt-product-id'),
            ...
            _.each(this.cache.priceObject, function(value, key) {
                var parentElementUpdate = $('[name="' + key + '"]').closest('li'),
                    productIdUpdate = parentElementUpdate.find('.related-checkbox').attr('data-mageants-fbt-product-id');
        ...
        _reloadImageProductList: function($this, checked) {
            var productId = $this.attr('data-mageants-fbt-product-id');
            var elm = $('.mageants-fbt-image-box li:not(".mageants-fbt-hidden"):first');
        },
        ...
        _reloadTotalPrice: function() {
            ...
            $('.mageants-fbt-rows .related-checkbox').each(function() {
                if ($(this).is(':checked')) {
            ...
            var priceBox = $('.mageants-fbt-price-box');
            $('.mageants-fbt-price-wrapper').attr('data-price-amount', totalPrice);
            $('.mageants-fbt-price').empty().append(_this._getFormattedPrice(totalPrice));
        ...
        _reloadButtonLabel: function(number) {
            ...
            $(".mageants-fbt-accessories-count").empty().append(numCount);

            // $('.mageants-fbt-add-to-cart button').attr('title', buttonCartLabel);
            // $('.mageants-fbt-add-to-cart button span').empty().append(buttonCartLabel);
        ...
        _reloadGroupedPrice: function($this) {
            var price = 0,
                _this = this,
                totalPrice = 0,
                productId = $this.closest('li').find('.related-checkbox').attr('data-mageants-fbt-product-id');
            if ($this.val() > 0) {
            ...
            $('.mageants-fbt-rows #mageants-fbt-super-product-table-' + productId + ' .mageants-fbt-grouped-qty').each(function() {
                totalPrice += parseFloat($(this).attr('data-child-product-price-total'));
            });
            $('.mageants-fbt-price-' + productId).attr('data-price-amount', totalPrice);
        ...
        _renderShowDetail: function() {
            var $widget = this;
            $('.mageants-fbt-rows .mageants-fbt-option-product').each(function() {
                var _this = this,
                    html = '';
                if ($(_this).children().length > 0) {
                    var element = $(_this).closest('li').find('.mageants-fbt-detail');
        ...
        _showHideDetail: function($this) {
            ...
            if (!$this.hasClass('not-active')) {
                ...
                $this.closest('li').find('.mageants-fbt-option-product').addClass('mageants-fbt-hidden');
            } else {
                $this.removeClass('not-active').empty().html($widget.options.hideDetails);
                $this.closest('li').find('.mageants-fbt-option-product').removeClass('mageants-fbt-hidden');
            }
        }
    });
    return $.mageants.frequentlyBought;
});